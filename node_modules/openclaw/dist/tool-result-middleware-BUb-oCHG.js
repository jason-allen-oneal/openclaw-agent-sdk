...
import { p as resolvePluginControlPlaneFingerprint } from "./current-plugin-metadata-snapshot-CjeTePSl.js";
import "./defaults-mDjiWzE5.js";
import { u as resolveProviderRuntimePlugin } from "./provider-hook-runtime-Cwefeem_.js";
import { o as normalizeProviderId } from "./model-selection-normalize-CBjx3j0e.js";
import "./model-selection-CRIOhEQm.js";
import { _t as generateSummary$1 } from "./sessions-C_fxanxf.js";
import { a as buildStageSplitPlan, i as buildOversizedFallbackPlan, l as sanitizeCompactionMessages, o as buildSummaryChunks, r as buildHistoryPrunePlan, s as computeAdaptiveChunkRatio } from "./compaction-planning-BD_0nYm1.js";
import { l as isTimeoutError } from "./failover-error-CC108ogz.js";
import { n as retryAsync } from "./retry-BSIArBBz.js";
import { f as isGoogleModelApi } from "./embedded-agent-helpers-CULGwdfn.js";
import { u as shouldPreserveThinkingBlocks } from "./provider-replay-helpers-DuODw6ME.js";
...
async function withTimeout(work, timeoutMs, label) {
	...
	timer.unref?.();
	let abortListener;
	const abortPromise = abortCtrl.signal.aborted ? Promise.reject(toLintErrorObject$1(abortCtrl.signal.reason ?? timeoutError, "Non-Error rejection")) : new Promise((_, reject) => {
		abortListener = () => reject(toLintErrorObject$1(abortCtrl.signal.reason ?? timeoutError, "Non-Error rejection"));
...
function resolveCompactionTimeoutMs(cfg) {
	return finiteSecondsToTimerSafeMilliseconds(cfg?.agents?.defaults?.compaction?.timeoutSeconds, { floorSeconds: true }) ?? 18e4;
}
...
function runCompactionPlanningWorker(params) {
	if (params.signal?.aborted) return Promise.reject(toLintErrorObject(params.signal.reason ?? /* @__PURE__ */ new Error("compaction planning aborted"), "Non-Error rejection"));
	...
	return new Promise((resolve, reject) => {
		...
		const abort = () => {
			settle(() => reject(toLintErrorObject(params.signal?.reason ?? /* @__PURE__ */ new Error("compaction planning aborted"), "Non-Error rejection")), true);
		};
...
function createAgentToolResultMiddlewareRunner(ctx, handlers) {
	...
	const resolvedHandlersLoader = createLazyPromiseLoader(async () => {
		const { loadAgentToolResultMiddlewaresForRuntime } = await import("./agent-tool-result-middleware-loader-DU0ZUE4N.js");
