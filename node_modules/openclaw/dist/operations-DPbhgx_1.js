...
const loadOverviewModule = async () => await import("./overview-DBBRS4_U.js");
const loadModelsSharedModule = async () => await import("./shared-SYbD3V3n.js");
const loadConfigCliModule = async () => await import("./config-cli-tAHO6UfJ.js");
const loadDoctorModule = async () => await import("./doctor-CUJwuBfL.js");
...
async function runGatewayLifecycle(operation) {
	const lifecycle = await import("./lifecycle-BHCQ_kMz.js");
	if (operation === "start") {
...
async function executeCrestodianOperation(operation, runtime, opts = {}) {
	...
	if (operation.kind === "plugin-search") {
		...
		await (opts.deps?.runPluginsSearch ?? (async (query, pluginRuntime) => {
			const { runPluginsSearchCommand } = await import("./plugins-search-command-hOaYCTLD.js");
	...
	if (operation.kind === "setup") {
		...
		const applyDefaultModelPrimaryUpdate = setupModel.model ? (await loadModelsSharedModule()).applyDefaultModelPrimaryUpdate : void 0;
	...
	if (operation.kind === "plugin-uninstall") {
		...
		await (opts.deps?.runPluginUninstall ?? (async (pluginId, pluginRuntime) => {
			const { runPluginUninstallCommand } = await import("./plugins-uninstall-command-C4PR9nId.js");
	...
	if (operation.kind === "create-agent") {
		...
		const before = await readConfigFileSnapshot();
		const workspace = resolveUserPath(operation.workspace ?? process.cwd());
		await (opts.deps?.runAgentsAdd ?? (await import("./agents.commands.add-B7YiUxkq.js")).agentsAddCommand)({
	...
	if (operation.kind === "doctor") {
		...
		await (opts.deps?.runDoctor ?? (await loadDoctorModule()).doctorCommand)(runtime, { nonInteractive: true });
	...
	if (operation.kind === "doctor-fix") {
		...
		const { readConfigFileSnapshot } = await loadConfigModule();
		const before = await readConfigFileSnapshot();
		await (opts.deps?.runDoctor ?? (await loadDoctorModule()).doctorCommand)(runtime, {
			nonInteractive: true,
			repair: true,
			yes: true
		});
	...
	if (operation.kind === "status") {
		...
		const { statusCommand } = await import("./status.command-C0jE5d1K.js");
	...
	if (operation.kind === "open-tui") {
		...
		const result = await (opts.deps?.runTui ?? (await import("./tui-BFjHteEW.js")).runTui)({
			local: true,
			session,
			deliver: false,
			historyLimit: 200
