...
import { n as projectOpenAITools } from "./openai-prompt-cache-fygROe7S.js";
import { c as isResponsesTextContentPartType, i as resolveOpenAIStrictToolSetting, n as normalizeOpenAIStrictToolParameters, r as resolveOpenAIStrictToolFlagForProjection, s as isAzureResponsesTextDeltaEvent, t as findOpenAIStrictToolProjectionDiagnostics } from "./openai-tool-schema-C7t-Si2L.js";
...
async function processResponsesStream(openaiStream, output, stream, model, options) {
	...
	for await (const event of openaiStream) if (event.type === "response.created") output.responseId = event.response.id;
		...
		if (item.type === "reasoning" && currentBlock?.type === "thinking") {
			...
			currentBlock.text = item.content.map((c) => c.type === "output_text" || c.type === "text" ? c.text : c.refusal).join("");
