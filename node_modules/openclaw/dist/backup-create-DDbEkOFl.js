...
import { f as resolveHomeDir, g as shortenHomePath, p as resolveUserPath, u as pathExists$1 } from "./utils-CCC-BEJH.js";
...
import { n as isPathWithin, t as buildCleanupPlan } from "./cleanup-utils-Bu92_0Ct.js";
...
async function resolveBackupPlanFromPaths(params) {
	...
	if (onlyConfig) {
		...
		if (!await pathExists$1(resolvedConfigPath)) return {
			stateDir,
			configPath,
			oauthDir,
			workspaceDirs: [],
	...
	const candidates = await Promise.all(rawCandidates.map(async (candidate) => {
		const exists = await pathExists$1(candidate.sourcePath);
...
async function createSanitizedStateSqliteBackupAsset(params) {
	...
	if (!await pathExists(archiveSourcePath)) return;
...
async function listAgentSqlitePaths(stateDir) {
	...
	async function visit(dir) {
		...
		for (const entry of entries) {
			...
			else if (entry.isFile() && entry.name === "openclaw-agent.sqlite") found.push(entryPath);
		}
	}
...
async function createAgentSqliteBackupAssets(params) {
	...
	for (const archiveSourcePath of sqlitePaths) {
		const source = new sqlite.DatabaseSync(archiveSourcePath, { readOnly: true });
		const sourcePath = path.join(params.tempDir, `openclaw-agent-backup-${snapshots.length}.sqlite`);
		try {
			source.exec("PRAGMA busy_timeout = 30000;");
			source.prepare("VACUUM INTO ?").run(sourcePath);
		} finally {
...
async function createBackupArchive(opts = {}) {
	...
	try {
		const sanitizedStateSqlite = stateAsset ? await createSanitizedStateSqliteBackupAsset({
			stateDir: stateAsset.sourcePath,
			tempDir
		}) : void 0;
		const agentSqliteSnapshots = stateAsset ? await createAgentSqliteBackupAssets({
			stateDir: stateAsset.sourcePath,
			tempDir
		}) : [];
		const sourcePathRemaps = /* @__PURE__ */ new Map();
		if (sanitizedStateSqlite) sourcePathRemaps.set(path.resolve(sanitizedStateSqlite.sourcePath), sanitizedStateSqlite.archiveSourcePath);
		for (const snapshot of agentSqliteSnapshots) sourcePathRemaps.set(path.resolve(snapshot.sourcePath), snapshot.archiveSourcePath);
		...
		const tarFilter = (entryPath) => {
			...
			if (sanitizedStateSqlite?.skippedSourcePaths.has(path.resolve(entryPath)) || agentSqliteSnapshots.some((snapshot) => snapshot.skippedSourcePaths.has(path.resolve(entryPath)))) return false;
