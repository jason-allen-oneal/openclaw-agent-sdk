import { n as isVitestRuntimeEnv } from "./env-DcqIKp5M.js";
import { t as isGatewayModelPricingEnabled } from "./model-pricing-config-pKJPI_G6.js";
import { r as startHeartbeatRunner } from "./heartbeat-runner-C0KdNCLL.js";
import { r as createNoopHeartbeatRunner } from "./server-runtime-startup-services-D6mFs7hH.js";
...
function recoverPendingOutboundDeliveries(params) {
	(async () => {
		const { recoverPendingDeliveries } = await import("./delivery-queue-C2J0aM1D.js");
		const { deliverOutboundPayloadsInternal } = await import("./deliver-BUgQHn7P.js");
...
function recoverPendingSessionDeliveries(params) {
	setTimeout(() => {
		(async () => {
			const { recoverPendingRestartContinuationDeliveries } = await import("./server-restart-sentinel-BHbV9zaa.js");
...
function startGatewayModelPricingRefreshOnDemand(params) {
	...
	let stopRefresh;
	(async () => {
		const { startGatewayModelPricingRefresh } = await import("./model-pricing-cache-DQqjEjr2.js");
		if (stopped) return;
