...
import { a as resolveOsHomeRelativePath } from "./home-dir-BjcCg_IW.js";
...
import { n as isImmutableGitCommitRef } from "./git-install-Ced3DGEX.js";
...
import { o as installPluginFromPath } from "./install-5B0zvvJg.js";
...
function marketplaceInstallPolicySource(params) {
	...
	if (resolveArchiveKind(params.resolvedPath)) {
		if (params.marketplaceOrigin === "remote" && params.source.kind === "path" && !isHttpUrl(params.source.path)) return {
			kind: "archive",
			authority: "third-party",
			mutable: marketplaceMutable,
			network: true
		};
		if (params.source.kind === "path" && !isHttpUrl(params.source.path)) return {
			kind: "archive",
			authority: "user",
			mutable: true,
			network: false
	...
	if (params.source.kind === "path") {
		if (isHttpUrl(params.source.path)) return {
			kind: "archive",
			authority: "third-party",
			mutable: true,
			network: true
...
async function loadMarketplace(params) {
	const loadMarketplaceFromManifestFile = async (paramsLocal) => {
		const parsed = parseMarketplaceManifest(await fs.readFile(paramsLocal.manifestPath, "utf-8"), paramsLocal.manifestPath);
		if (!parsed.ok) {
