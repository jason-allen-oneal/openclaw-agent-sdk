...
import { s as emitAgentEvent, t as assertAgentRunLifecycleGenerationCurrent } from "./agent-events-C3fRM85v.js";
import { o as requestHeartbeat } from "./heartbeat-wake-YhOjrxkC.js";
import { a as enqueueSystemEvent } from "./system-events--U8AE18s.js";
import { r as classifyFailoverReason } from "./errors-B4mplIiL.js";
import { d as resolveFailoverStatus, t as FailoverError } from "./failover-error-CC108ogz.js";
import { n as applySkillEnvOverridesFromSnapshot } from "./env-overrides-CbvqUyzX.js";
import { i as scopedHeartbeatWakeOptionsForPolicy, n as resolveEventSessionRoutingPolicy, t as resolveEventSessionKeyForPolicy } from "./event-session-routing-BKUkHfqC.js";
import "./embedded-agent-helpers-CULGwdfn.js";
import { t as shouldUseInternalSourceReplySink } from "./internal-source-reply-ByESZ0e7.js";
import { n as appendBootstrapPromptWarning } from "./bootstrap-budget-bmmzkOyn.js";
import { a as isMessagingToolSendAction, i as isMessagingToolDeliveryAction, o as isMessagingToolTargetEvidenceAction, r as isMessagingTool } from "./embedded-agent-messaging-DZP5HfHM.js";
import { a as extractMessagingToolSourceReplyPayload, i as extractMessagingToolSend, m as sanitizeToolResult, n as collectMessagingMediaUrlsFromRecord, p as sanitizeToolArgs, r as collectMessagingMediaUrlsFromToolResult } from "./embedded-agent-subscribe.tools-B2A6MVZq.js";
...
import { i as formatCliBackendOutputDigest, r as cliBackendLog } from "./log-B_qvw8_q.js";
import { h as waitForMcpLoopbackToolCallCaptureIdle, r as clearMcpLoopbackToolCallCapture, t as beginMcpLoopbackToolCallCapture } from "./mcp-http.loopback-runtime-D3ur4BUo.js";
import { a as shouldUseClaudeLiveSession, c as parseCliOutput, i as runClaudeLiveSessionTurn, o as createCliJsonlStreamingParser, r as rotateClaudeLiveMcpCaptureKeyForContext, s as extractCliErrorMessage } from "./claude-live-session-BW9CCup1.js";
import { n as prepareCliBundleMcpCaptureAttempt, t as prepareClaudeCliSkillsPlugin } from "./claude-skills-plugin-BaYyrrh1.js";
import { c as resolvePromptInput, d as writeCliSystemPromptFile, f as buildCliSupervisorScopeKey, i as enqueueCliRun, l as resolveSessionIdToSend, m as resolveCliRunTimeoutOverrideMs, o as prepareCliPromptImagePayload, p as resolveCliNoOutputTimeoutMs, r as buildCliArgs, s as resolveCliRunQueueKey, t as buildClaudeOwnerKey, u as resolveSystemPromptUsage } from "./helpers-BmplemDu.js";
...
function normalizeCliMessagingToolName(toolName) {
...
function extractCliMessagingTarget(context, toolName, args) {
	...
	return extractMessagingToolSend(normalizedToolName, targetArgs, { currentChannelId: context.params.currentChannelId });
}
...
async function executePreparedCliRun(context, cliSessionIdToUse) {
	...
	const systemPromptFile = systemPromptArg && (!useResume || backend.systemPromptWhen === "always") ? await executeDeps.writeCliSystemPromptFile({
		backend,
	...
	const fallbackClaudeSkillsPlugin = context.claudeSkillsPluginArgs === void 0 ? await prepareClaudeCliSkillsPlugin({
		backendId: context.backendResolved.id,
		skillsSnapshot: params.skillsSnapshot
	}) : void 0;
	let fallbackClaudeSkillsPluginCleanupOwned = false;
	const claudeSkillsPluginArgs = context.claudeSkillsPluginArgs ?? fallbackClaudeSkillsPlugin?.args ?? [];
	...
	try {
		completedOutput = await enqueueCliRun(queueKey, async () => {
			...
			const withMessagingDeliveryEvidence = (output) => {
				return {
					...output,
			...
			try {
				...
				const initialGatewayCaptureKey = shouldUseClaudeLiveSession(context) ? void 0 : buildCliMcpCaptureKey(context);
				...
				if (logOutputText) {
					...
					if (env.OPENCLAW_MCP_TOKEN || env.OPENCLAW_MCP_SESSION_KEY || env.OPENCLAW_MCP_AGENT_ID) cliBackendLog.info(`cli env mcp: ${buildCliEnvMcpLog(env)}`);
				}
					...
					if (isMessagingSend) {
						...
						if (isDeliveredMessageToolOnlySourceReplyResult({
							...
							if (sourceReplyPayload) {
								if (messagingToolSourceReplyPayloads.length >= CLI_MESSAGING_EVIDENCE_MAX_CALLS) messagingToolSourceReplyPayloads.shift();
				...
				const beginGatewayCapture = (captureKey) => {
					...
					beginMcpLoopbackToolCallCapture({
						...
						onToolCallResult: ({ toolName, args: toolArgs, result, isError }) => {
							const normalizedToolName = normalizeCliMessagingToolName(toolName);
							if (!isMessagingToolDeliveryAction(normalizedToolName, toolArgs)) return;
				...
				const emitCliToolUseStart = (event) => {
					...
					if (!gatewayCaptureKey && event.args.dryRun !== true && isMessagingToolDeliveryAction(toolName, event.args)) {
				...
				if (shouldUseClaudeLiveSession(context)) {
					...
					const ownedPreparedBackendCleanup = liveSessionOwnsRunArtifacts ? context.preparedBackend.cleanup : void 0;
					if (liveSessionOwnsRunArtifacts) context.preparedBackend.cleanup = void 0;
					const liveResult = await runClaudeLiveSessionTurn({
						...
						cleanup: liveSessionOwnsRunArtifacts ? async () => {
							try {
					...
					if (result.exitCode !== 0 || result.reason !== "exit") {
						...
						const retryCode = reason === "unknown" && result.reason === "exit" && errorCandidates.length === 0 && !observedCliActivity ? "cli_unknown_empty_failure" : void 0;
				...
				try {
					...
					if (gatewayCaptureKey) {
						if (!await waitForMcpLoopbackToolCallCaptureIdle(gatewayCaptureKey, {
							...
							if (shouldUseClaudeLiveSession(context)) await rotateClaudeLiveMcpCaptureKeyForContext(context);
					...
					if (gatewayCaptureKey) clearMcpLoopbackToolCallCapture(gatewayCaptureKey);
				}
				try {
