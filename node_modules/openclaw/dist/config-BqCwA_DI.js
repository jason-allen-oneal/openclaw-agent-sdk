...
import "./types.secrets-B6pb_3Jn.js";
import { n as replaceFileAtomic } from "./replace-file-BrS02dAb.js";
import "./scan-paths-Bve2UhXh.js";
import { E as validateConfigObjectWithPlugins, K as createInvalidConfigError, V as applyUnsetPathsForWrite, W as resolveManagedUnsetPathsForWrite, Y as maintainConfigBackups, b as restoreEnvChangesIfUnchanged, d as readConfigFileSnapshotForWrite, q as formatInvalidConfigDetails, x as writeConfigFile, y as resolveConfigSnapshotHash } from "./io-fQmSDfK-.js";
...
async function readFileRawIfExists(filePath) {
	try {
...
async function rollbackJsonFileWriteIfUnchanged(params) {
	if (hashFileRaw(await readFileRawIfExists(params.filePath)) !== params.committedHash) return false;
...
async function tryWriteSingleTopLevelIncludeMutation(params) {
	...
	const committedIncludeHash = hashFileRaw(formatJsonFileValue(nextConfigRecord[key]));
	...
	try {
		...
		let refreshed;
		try {
			refreshed = await (params.io?.readConfigFileSnapshotForWrite ?? readConfigFileSnapshotForWrite)(params.writeOptions?.skipPluginValidation ? { skipPluginValidation: true } : void 0);
		} finally {
