...
import { h as resolveLegacyStateDirs, y as resolveStateDir } from "./paths-mvMm5bYV.js";
...
import { a as isSubagentSessionKey, i as isCronSessionKey } from "./session-key-utils-Bx3apsJ3.js";
...
async function hasSkipBootstrapWorkspaceContentEvidence(dir) {
	try {
		...
		for (const entry of entries) {
			if (entry.name === ".DS_Store" || entry.name === WORKSPACE_STATE_DIRNAME) continue;
...
function resolveWorkspaceAttestationPath(dir) {
...
async function isWorkspaceAttestationMarker(attestationPath) {
...
async function shouldRemoveWorkspaceAttestation(attestationPath, opts) {
	try {
		return await isWorkspaceAttestationMarker(attestationPath) || await hasRecentWorkspaceAttestation(attestationPath, opts);
	} catch {
		return false;
	}
}
...
async function ensureAgentWorkspace(params) {
	...
	if (recentAttestationPath && !isBrandNewWorkspace) {
		...
		const state = await readWorkspaceSetupState(statePath, { persistLegacyMigration: true });
...
//#endregion
export { resolveWorkspaceBootstrapStatus as C, resolveWorkspaceTemplateSearchDirs as E, resolveWorkspaceAttestationPaths as S, resolveWorkspaceTemplateDir as T, loadExtraBootstrapFiles as _, DEFAULT_MEMORY_FILENAME as a, reconcileWorkspaceBootstrapCompletion as b, DEFAULT_USER_FILENAME as c, ensureAgentWorkspace as d, filterBootstrapFilesForSession as f, isWorkspaceSetupCompleted as g, isWorkspaceBootstrapPending as h, DEFAULT_IDENTITY_FILENAME as i, WORKSPACE_VANISHED_ERROR_CODE as l, isWorkspaceAttestationMarker as m, DEFAULT_BOOTSTRAP_FILENAME as n, DEFAULT_SOUL_FILENAME as o, hasRecentWorkspaceAttestation as p, DEFAULT_HEARTBEAT_FILENAME as r, DEFAULT_TOOLS_FILENAME as s, DEFAULT_AGENTS_FILENAME as t, WorkspaceVanishedError as u, loadExtraBootstrapFilesWithDiagnostics as v, shouldRemoveWorkspaceAttestation as w, resolveWorkspaceAttestationPath as x, loadWorkspaceBootstrapFiles as y };
