...
import { i as normalizeEnv, t as isTruthyEnvValue } from "../env-DcqIKp5M.js";
import { C as getCoreCliParentDefaultHelpCommands, O as consumeRootOptionToken, _ as getSubCliEntries, c as hasFlag, m as normalizeRootHelpTargetArgv, p as normalizeGeneratedHelpCommandArgv, v as getSubCliParentDefaultHelpCommands, x as getCoreCliCommandNames } from "../argv-BulIeC99.js";
...
import { i as parseCliContainerArgs, n as parseCliProfileArgs, r as maybeRunCliInContainer, t as applyCliProfileEnv } from "../profile-BSorXH17.js";
...
import { a as shouldSkipPluginCommandRegistration, r as shouldRegisterPrimaryCommandOnly, t as isReservedNonPluginCommandRoot } from "../command-registration-policy-DYVPNJqV.js";
import { a as consumeGatewayFastPathRootOptionToken, n as resolveCliNetworkProxyPolicy, o as consumeGatewayRunOptionToken, t as resolveCliCommandPathPolicy } from "../command-path-policy-u_XH8zQZ.js";
...
const loadLoggingModule = async () => await import("../logging-CYdhxIMc.js");
const loadCliRegistryLoaderModule = async () => await import("../cli-registry-loader-BxHXT_wD.js");
const loadManifestCommandAliasesRuntimeModule = async () => await import("../manifest-command-aliases.runtime-CLDmyyOB.js");
const loadProxyLifecycleModule = async () => await import("../proxy-lifecycle-BhMJ9MGL.js");
...
async function tryRunGatewayRunFastPath(argv, startupTrace) {
	...
	const [{ Command }, { addGatewayRunCommand }, { VERSION }, { emitCliBanner }, { resolveCliStartupPolicy }, { enableConsoleCapture }] = await startupTrace.measure("gateway-run-imports", () => Promise.all([
		import("commander"),
		import("../run-command-CcZRTM8S.js"),
		import("../version-Dpd_K_tD.js"),
		import("../banner-tH_YaeAU.js"),
		import("../command-startup-policy-C0hAFqwD.js"),
		loadLoggingModule()
	]));
...
async function closeCliMemoryManagers() {
	try {
		...
		const { closeActiveMemorySearchManagers } = await import("../memory-runtime-DBcZKR01.js");
		await closeActiveMemorySearchManagers();
	} catch {}
}
async function disposeCliAgentHarnesses() {
	try {
		const { listAgentHarnessIds, disposeRegisteredAgentHarnesses } = await import("../registry-BuQH-4g3.js");
...
async function bootstrapCliProxyCaptureAndDispatcher(startupTrace, options = {}) {
	const [{ initializeDebugProxyCapture, finalizeDebugProxyCapture }, { maybeWarnAboutDebugProxyCoverage }] = await startupTrace.measure("proxy-imports", () => Promise.all([import("../runtime-CSl3EITs.js"), import("../coverage-p3FrgSmg.js")]));
...
async function runCli(argv = process$1.argv) {
	...
	if (!isHelpOrVersionInvocation && shouldLoadCliDotEnv()) await startupTrace.measure("dotenv", async () => {
		if (isRemoteAgentDispatchInvocation(normalizedArgv, normalizedInvocation.primary)) {
			const { loadGatewayDispatchCliDotEnv } = await import("../gateway-dispatch-dotenv--d8sN1G1.js");
			await loadGatewayDispatchCliDotEnv({ quiet: true });
		} else {
			const { loadCliDotEnv } = await import("../dotenv-DO5chJpc.js");
	...
	const readBestEffortCliConfig = async () => {
		if (!bestEffortConfigPromise) bestEffortConfigPromise = import("../io-MZVHbWrJ.js").then(({ readBestEffortConfig }) => readBestEffortConfig());
	...
	try {
		if (shouldUseRootHelpFastPath(normalizedArgv)) {
			...
			const { outputRootHelp } = await import("../root-help-BFlxhjdz.js");
		...
		if (shouldUseSetupOnboardConfigureHelpFastPath(normalizedArgv)) {
			const { tryOutputSetupOnboardConfigureHelp } = await import("../setup-onboard-configure-help-fast-path-CZp7WREU.js");
		...
		if (shouldRunBareRootCrestodian) {
			if (await shouldStartOnboardingForFreshInstall(normalizedArgv)) {
				...
				const { setupWizardCommand } = await import("../onboard-D5MBBPY2.js");
		...
		if (bootstrapProxyBeforeFastPath && await tryRunGatewayRunFastPath(normalizedArgv, startupTrace)) return;
		const { tryRouteCli } = await startupTrace.measure("route-import", () => import("../route-CYfVJgDA.js"));
		...
		try {
			...
			const [{ buildProgram }, { formatUncaughtError }, { formatCliFailureLines }, { runFatalErrorHooks }, { installUnhandledRejectionHandler, isBenignUncaughtExceptionError, isUncaughtExceptionHandled }, { restoreTerminalState }] = await startupTrace.measure("core-imports", () => Promise.all([
				import("../program-Kl7yb6iz.js"),
				import("../infra/errors.js"),
				import("../failure-output-BiitHn0T.js"),
				import("../fatal-error-hooks-BiXaGF_Q.js"),
				import("../unhandled-rejections-CLKv7ZCu.js"),
			...
			if (primary && shouldRegisterPrimaryCommandOnly(parseArgv)) await startupTrace.measure("register-primary", async () => {
				...
				if (ctx) {
					const { registerCoreCliByName } = await import("../command-registry-BJAxBOgS.js");
					await registerCoreCliByName(program, ctx, primary, parseArgv);
				}
				const { registerSubCliByName } = await import("../register.subclis-BLheUmEj.js");
			...
			if (!shouldSkipPluginCommandRegistration({
				...
				const config = await startupTrace.measure("register-plugin-commands", async () => {
					const { registerPluginCliCommandsFromValidatedConfig } = await import("../cli-Dmx3lK_u.js");
