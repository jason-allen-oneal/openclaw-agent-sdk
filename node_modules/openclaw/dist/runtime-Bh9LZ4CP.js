...
import { d as resolveCommitmentTimezone, f as resolveCommitmentsConfig, i as listPendingCommitmentsForScope, u as upsertInferredCommitments } from "./store-Bdr470Sd.js";
...
async function defaultExtractBatch(params) {
	...
	const { runEmbeddedAgent } = await import("./embedded-agent-MCTquNH9.js");
	...
	try {
		...
		while (queue.length > 0) {
			...
			try {
				...
				if (isTerminalExtractionError(error)) openTerminalFailureCooldown(items[0]?.agentId ?? "", error, Date.now(), items[0]?.nowMs ?? Date.now());
				throw error;
			}
