...
import { g as resolveConfiguredModelRef, r as buildConfiguredModelCatalog } from "./model-selection-shared-By-X2JmI.js";
...
import { t as collectEnabledInsecureOrDangerousFlagsFromCurrentSnapshot } from "./dangerous-config-flags-current-BfGgM9d2.js";
...
async function logGatewayStartup(params) {
	...
	const enabledDangerousFlags = collectEnabledInsecureOrDangerousFlagsFromCurrentSnapshot(params.cfg) ?? (await import("./dangerous-config-flags-DFsK3533.js")).collectEnabledInsecureOrDangerousFlags(params.cfg);
...
function formatAgentModelStartupDetails(params) {
	...
	return `thinking=${explicitThinking ?? (isConfiguredReasoningDisabled({
		...
		provider: params.provider,
		model: params.model
	}) ? "off" : resolvedThinking === "off" ? "medium" : resolvedThinking)}, fast=${resolveFastModeState({
		cfg: params.cfg,
		provider: params.provider,
		model: params.model,
