...
import { a as getCachedLiveProviderModelRows, t as LiveModelCatalogHttpError } from "./provider-catalog-live-runtime-CfJaGj4F.js";
...
function isRetryableVeniceDiscoveryError(err) {
	if (err instanceof LiveModelCatalogHttpError) return VENICE_DISCOVERY_RETRYABLE_HTTP_STATUS.has(err.status);
...
async function discoverVeniceModels(options = {}) {
	...
	try {
		const data = await retryAsync(async () => await getCachedLiveProviderModelRows({
			providerId: "venice",
