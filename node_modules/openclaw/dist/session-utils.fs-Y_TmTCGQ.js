import { a as normalizeLowercaseStringOrEmpty, c as normalizeOptionalString$1, s as normalizeOptionalLowercaseString } from "./string-coerce-mnp54Vah.js";
import { D as resolveIntegerOption, O as resolveNonNegativeIntegerOption } from "./number-coercion-CJQ8TR--.js";
import { t as escapeRegExp$1 } from "./regexp-BZyMFTlj.js";
...
import { i as hasInterSessionUserProvenance } from "./input-provenance-CYQvfQZP.js";
...
import { a as resolveSessionTranscriptCandidates, o as resolveSessionTranscriptResetArchiveCandidatesAsync } from "./session-transcript-files.fs-BoQs4wK-.js";
...
function extractJsonStringFieldPrefix$1(prefix, field) {
...
function extractJsonNullableStringFieldPrefix$1(prefix, field) {
	if (new RegExp(`"${escapeRegExp(field)}"\\s*:\\s*null`).test(prefix)) return null;
	return extractJsonStringFieldPrefix$1(prefix, field);
}
function extractJsonNumberFieldPrefix$1(prefix, field) {
...
function buildOversizedIndexedRawEntry(params) {
	...
	const id = extractJsonStringFieldPrefix$1(prefix, "id");
	const parentId = extractJsonNullableStringFieldPrefix$1(prefix, "parentId");
	const type = extractJsonStringFieldPrefix$1(prefix, "type");
	const timestamp = extractJsonStringFieldPrefix$1(recordPrefix, "timestamp") ?? extractJsonNumberFieldPrefix$1(recordPrefix, "timestamp");
	const role = extractJsonStringFieldPrefix$1(prefix, "role") ?? "assistant";
	const record = {
		...type ? { type } : {},
		...id ? { id } : {},
...
function extractJsonStringFieldPrefix(prefix, field) {
	const match = new RegExp(`"${escapeRegExp$1(field)}"\\s*:\\s*"((?:\\\\.|[^"\\\\])*)"`).exec(prefix);
	if (!match) return;
	try {
...
function extractJsonNullableStringFieldPrefix(prefix, field) {
	if (new RegExp(`"${escapeRegExp$1(field)}"\\s*:\\s*null`).test(prefix)) return null;
	return extractJsonStringFieldPrefix(prefix, field);
}
function extractJsonNumberFieldPrefix(prefix, field) {
	const match = new RegExp(`"${escapeRegExp$1(field)}"\\s*:\\s*(-?\\d+(?:\\.\\d+)?(?:[eE][+-]?\\d+)?)`).exec(prefix);
	if (!match) return;
	const decoded = Number(match[1]);
	return Number.isFinite(decoded) ? decoded : void 0;
}
...
async function readSessionMessagesAsync(sessionId, storePath, sessionFile, opts) {
	return (await readSessionMessagesWithSourceAsync(sessionId, storePath, sessionFile, opts)).messages;
}
async function readSessionMessagesWithSourceAsync(sessionId, storePath, sessionFile, opts) {
	if (opts.mode === "recent") {
		const { mode: _modeValue, ...recentOpts } = opts;
		return await readRecentSessionMessagesWithSourceAsync(sessionId, storePath, sessionFile, recentOpts);
	}
	const filePath = opts.allowResetArchiveFallback === true ? await findExistingTranscriptHistoryPathAsync(sessionId, storePath, sessionFile) : findExistingTranscriptPath(sessionId, storePath, sessionFile);
...
async function readRecentSessionMessagesAsync(sessionId, storePath, sessionFile, opts) {
	return (await readRecentSessionMessagesWithSourceAsync(sessionId, storePath, sessionFile, opts)).messages;
}
async function readRecentSessionMessagesWithSourceAsync(sessionId, storePath, sessionFile, opts) {
...
//#endregion
export { stripInternalMetadataForDisplay as C, extractToolCallNames as S, stripMessageIdHints as T, visitSessionMessagesAsync as _, readRecentSessionMessagesWithStatsAsync as a, stripEnvelopeFromMessages as b, readSessionMessageByIdAsync as c, readSessionMessagesAsync as d, readSessionMessagesWithSourceAsync as f, resolveSessionHistoryTranscriptPathAsync as g, readSessionTitleFieldsFromTranscriptAsync as h, readRecentSessionMessagesAsync as i, readSessionMessageCountAsync as l, readSessionTitleFieldsFromTranscript as m, capArrayByJsonBytes as n, readRecentSessionTranscriptLines as o, readSessionPreviewItemsFromTranscript as p, readLatestRecentSessionUsageFromTranscriptAsync as r, readRecentSessionUsageFromTranscript as s, attachOpenClawTranscriptMeta as t, readSessionMessages as u, readSessionTranscriptIndex as v, stripEnvelope as w, countToolResults as x, stripEnvelopeFromMessage as y };
