...
function ensureWorkboardSchema(db) {
  ...
  `);
	...
	db.prepare("INSERT OR IGNORE INTO workboard_schema_migrations (id, applied_at) VALUES (?, ?)").run(`schema-${SCHEMA_VERSION}`, Date.now());
}
function configureWorkboardDatabase(db) {
	db.exec(`
...
function readExecution(row) {
	...
	return {
		id,
		kind: "agent-session",
		engine: requiredString(row, "execution_engine"),
