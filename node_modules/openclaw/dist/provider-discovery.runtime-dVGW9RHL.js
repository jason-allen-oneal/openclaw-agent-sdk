...
import { n as getCachedPluginModuleLoader, s as clearNativeRequireJavaScriptModuleCache, t as createPluginModuleLoaderCache } from "./plugin-module-loader-cache-lz0qhu-X.js";
...
import { a as resolveDiscoveredProviderPluginIds } from "./providers-vBGl4iTG.js";
...
function resolveRuntimeManifestCatalogPluginIds(pluginRecords) {
	...
	for (const plugin of pluginRecords) {
		...
		if (Object.entries(plugin.modelCatalog?.discovery ?? {}).some(([provider, discovery]) => discovery === "runtime" && ownedProviders.has(normalizeProviderId(provider)))) pluginIds.add(plugin.id);
		if (!plugin.modelCatalog?.providers) continue;
		if (planManifestModelCatalogRows({ registry: { plugins: [plugin] } }).entries.some((entry) => entry.discovery === "runtime")) pluginIds.add(plugin.id);
	}
	return pluginIds;
}
