...
import "./prompt-B3hgTDNT.js";
import { i as runExec } from "./exec-iwN9d-_i.js";
import "./binaries-ByjAa1pC.js";
...
async function findTailscaleBinary() {
	const checkBinary = async (path) => {
		...
		try {
			await Promise.race([runExec(path, ["--version"], { timeoutMs: 3e3 }), new Promise((_, reject) => {
				setTimeout(() => reject(/* @__PURE__ */ new Error("timeout")), 3e3);
			})]);
			return true;
		} catch {
			return false;
...
async function getTailnetHostname(exec = runExec, detectedBinary) {
	...
	throw toLintErrorObject(lastError ?? /* @__PURE__ */ new Error("Could not determine Tailscale DNS or IP"), "Non-Error thrown");
}
/**
