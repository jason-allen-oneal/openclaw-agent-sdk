...
import { i as isMessagingToolDeliveryAction, n as isMessageToolSendActionName, t as isMessageToolConversationCreateActionName } from "./embedded-agent-messaging-DZP5HfHM.js";
import { d as isToolResultError } from "./embedded-agent-subscribe.tools-B2A6MVZq.js";
...
function deliveryEnvelopeIndicatesDelivered(value, depth = 0) {
	...
	const record = value;
	if (normalizeStatus(record.deliveryStatus) === SENT_DELIVERY_STATUS || normalizeStatus(record.status) === SENT_DELIVERY_STATUS || recordHasDeliveredMessageId(record)) return true;
	if (typeof record.text === "string") {
		const parsed = parseJsonRecord(record.text);
...
//#endregion
export { isDeliveredMessagingToolResult as n, isDeliveredMessageToolOnlySourceReplyResult as t };
