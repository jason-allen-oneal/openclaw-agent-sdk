...
import "../../api-Uu6GS9j1.js";
import { n as dispatchAndStartWorkboardCards, r as WorkboardStore, t as registerWorkboardGatewayMethods } from "../../runtime-api-5hMw6AHk.js";
...
function createWorkboardTools(params) {
	...
	return [
		...
		},
		{
			...
			description: "Attach proof or artifact metadata to a Workboard card after running tests, checks, or producing screenshots/logs.",
			parameters: Type.Object({
				id: cardIdField(),
			...
			execute: async (_toolCallId, rawParams) => {
				const { record, id, scope } = await readScopedCardToolParams(rawParams);
				return redactedCardResult(typeof record.artifactPath === "string" && record.artifactPath.trim() !== "" || typeof record.url === "string" && record.url.trim() !== "" ? await store.addProofWithArtifact(id, record, {
					label: record.label,
		...
		},
		{
			...
			description: "Run one Workboard dispatcher pass: promote unblocked cards, reclaim expired claims, and block timed-out runs.",
			parameters: Type.Object({ boardId: Type.Optional(Type.String({ description: "Optional board id filter." })) }, { additionalProperties: false }),
			execute: async (_toolCallId, rawParams) => {
...
var workboard_default = definePluginEntry({
	...
	register(api) {
		...
		api.registerCli(async ({ program }) => {
			const { registerWorkboardCli } = await import("../../cli-D90w1uNl.js");
		...
		api.registerTool((context) => createWorkboardTools({
			api,
			context,
			store
		}), {
