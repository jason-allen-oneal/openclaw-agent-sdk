...
import { C as resolveExpiresAtMsFromDurationMs, b as parseStrictPositiveInteger, o as asDateTimestampMs } from "../../number-coercion-CJQ8TR--.js";
...
import { o as resolveAgentEffectiveModelPrimary } from "../../agent-scope-B4NxGkqZ.js";
import { c as parseAgentSessionKey, u as parseThreadSessionSuffix } from "../../session-key-utils-Bx3apsJ3.js";
...
import { t as definePluginEntry } from "../../plugin-entry-EMYYC62u.js";
import "../../routing-CqTr30kc.js";
import { n as resolveLivePluginConfigObject, r as resolvePluginConfigObject } from "../../plugin-config-runtime-CY3KqaE_.js";
import "../../agent-runtime-GsSfjDVv.js";
import { t as closeActiveMemorySearchManager } from "../../memory-host-search-CLSo6JE1.js";
...
function setCachedResult(cacheKey, result, ttlMs) {
	...
	if (activeRecallCache.size >= DEFAULT_MAX_CACHE_ENTRIES || now !== void 0 && now - lastActiveRecallCacheSweepAt >= CACHE_SWEEP_INTERVAL_MS) {
...
async function readActiveMemoryTranscriptState(sessionFile, limits, toolsAllow) {
	let searchDebug;
...
async function readMergedActiveMemoryTranscriptState(params) {
	...
	for (const sessionFile of new Set(params.sessionFiles)) {
		const state = await readActiveMemoryTranscriptState(sessionFile, void 0, params.toolsAllow);
...
async function readTerminalMemorySearchResult(sessionFile, limits, toolsAllow) {
...
function watchTerminalMemorySearchResult(params) {
	...
	const tick = async () => {
		...
		try {
			...
			const result = sessionFile ? await readTerminalMemorySearchResult(sessionFile, void 0, params.toolsAllow) : void 0;
			if (result) {
				finish(result);
				return;
			}
...
async function buildTimeoutRecallResult(params) {
	...
	const transcriptState = params.sessionFile ? await readActiveMemoryTranscriptState(params.sessionFile, void 0, params.toolsAllow) : void 0;
...
async function runRecallSubagent(params) {
	...
	try {
		...
		return {
			...
			transcriptPath: params.config.persistTranscripts ? activeSessionFile : void 0,
			searchDebug,
		...
		if (params.abortSignal?.aborted) {
			...
			const transcriptState = await readActiveMemoryTranscriptState(activeSessionFile, void 0, params.config.toolsAllow);
		...
		api.on("before_prompt_build", async (event, ctx) => {
			...
			const handlerPromise = (async () => {
				try {
					...
					if (!isAllowedChatType(invocationConfig, {
						...ctx,
					...
					if (!isAllowedChatId(invocationConfig, {
						sessionKey: resolvedSessionKey ?? ctx.sessionKey,
