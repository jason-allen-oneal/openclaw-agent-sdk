...
async function startGatewayBonjourAdvertiser(opts, deps = {}) {
	...
	try {
		...
		async function stopCycle(cycle, optsValue) {
			if (!cycle) return;
			for (const { svc } of cycle.services) try {
				await svc.destroy();
			} catch {}
			try {
				if (optsValue?.shutdownResponder) await cycle.responder.shutdown();
			} catch {}
		}
		...
		const recreateAdvertiser = async (reason, optsLocal) => {
			if (stopped || disabled) return;
			if (recreatePromise) return recreatePromise;
			recreatePromise = (async () => {
				consecutiveRestarts += 1;
				consecutiveStuckStateRestarts = optsLocal?.stuckState ? consecutiveStuckStateRestarts + 1 : 0;
				const now = Date.now();
