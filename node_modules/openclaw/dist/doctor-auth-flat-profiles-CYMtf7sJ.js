...
import { a as resolveAgentDir, n as listAgentIds, s as resolveDefaultAgentDir } from "./agent-scope-config-DNJywrQV.js";
import { d as resolveLegacyAuthStorePath, l as resolveAuthStorePath, s as resolveAuthStatePath } from "./runtime-snapshots-B1wdikN8.js";
import { _ as loadLegacyAuthProfileStore, g as coercePersistedAuthProfileStore, h as applyLegacyAuthStore, p as saveAuthProfileStore, t as clearRuntimeAuthProfileStoreSnapshots, v as loadPersistedAuthProfileStore, y as coerceAuthProfileState } from "./store-jnx0nhpN.js";
...
function mergeImportedAuthProfiles(params) {
	...
	for (const [profileId, credential] of Object.entries(params.profiles)) if (!params.existingProfileIds.has(profileId)) profiles[profileId] = credential;
	return {
		...params.store,
		profiles
	};
...
async function maybeMigrateAuthProfileJsonStoresToSqlite(params) {
	...
	const detected = listAuthProfileSqliteMigrationCandidates(params.cfg, env).filter((candidate) => fs.existsSync(candidate.authPath) || fs.existsSync(candidate.statePath) || fs.existsSync(candidate.legacyPath));
	const result = {
	...
	for (const candidate of detected) try {
		...
		if (canonicalStore || legacyStore || hasAuthProfileState(state)) {
			const stateProfileIds = [...collectAuthProfileStateProfileIds(state), ...canonicalStore ? collectAuthProfileStateProfileIds(coerceAuthProfileState(canonicalStore)) : []];
		...
		result.changes.push(`Migrated auth profile JSON for ${shortenHomePath(candidate.authPath)} into SQLite (backup${backups.length === 1 ? "" : "s"}: ${backups.map(shortenHomePath).join(", ")}).`);
