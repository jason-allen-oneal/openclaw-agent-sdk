...
const OPENCLAW_STABLE_CORRECTION_VERSION_RE = /^(?<year>\d{4})\.(?<month>[1-9]\d?)\.(?<patch>[1-9]\d*)-(?<correction>[1-9]\d*)$/;
const OPENCLAW_STABLE_VERSION_RE = /^(?<year>\d{4})\.(?<month>[1-9]\d?)\.(?<patch>[1-9]\d*)$/;
const OPENCLAW_ALPHA_VERSION_RE = /^(?<year>\d{4})\.(?<month>[1-9]\d?)\.(?<patch>[1-9]\d*)-alpha\.(?<alpha>[1-9]\d*)$/;
const OPENCLAW_BETA_VERSION_RE = /^(?<year>\d{4})\.(?<month>[1-9]\d?)\.(?<patch>[1-9]\d*)-beta\.(?<beta>[1-9]\d*)$/;
...
function parseOpenClawReleaseVersion(value) {
	...
	if (!Number.isInteger(year) || !Number.isInteger(month) || !Number.isInteger(patch) || month < 1 || month > 12 || patch < 1) return null;
...
function compareOpenClawReleaseVersions(left, right) {
	...
	if (parsedLeft.year !== parsedRight.year) return parsedLeft.year < parsedRight.year ? -1 : 1;
	if (parsedLeft.month !== parsedRight.month) return parsedLeft.month < parsedRight.month ? -1 : 1;
