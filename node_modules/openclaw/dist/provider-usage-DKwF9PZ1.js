...
import { _ as parseStrictFiniteNumber, g as parseFiniteNumber$1, j as resolveTimerTimeoutMs, o as asDateTimestampMs } from "./number-coercion-CJQ8TR--.js";
...
async function fetchDeepSeekUsage(apiKey, timeoutMs, fetchFn) {
	...
	if (!parsed.ok) return parsed.snapshot;
	const data = parsed.data;
	const summary = (Array.isArray(data.balance_infos) ? data.balance_infos : []).map((info) => buildBalanceSummary(info)).filter((entry) => Boolean(entry)).join(" · ");
	if (!summary) return {
		provider: "deepseek",
