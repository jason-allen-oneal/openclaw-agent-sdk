...
import { d as executeSqliteQuerySync, f as executeSqliteQueryTakeFirstSync, i as openOpenClawStateDatabase, o as runOpenClawStateWriteTransaction, p as getNodeSqliteKysely } from "./openclaw-state-db-J6qYHGxK.js";
import { t as parseByteSize } from "./parse-bytes-00heCtL_.js";
import { c as cronStoreKey } from "./normalize-Da7MTCo5.js";
import { u as resolveFailoverReasonFromError } from "./failover-error-CC108ogz.js";
import { i as normalizeCronRunDiagnostics } from "./run-diagnostics-0rGfYsxM.js";
...
function countCronRunLogRows(params) {
	return normalizeNumber(executeSqliteQueryTakeFirstSync(params.db, applyRunLogFilters(getCronRunLogKysely(params.db).selectFrom("cron_run_logs").select((eb) => eb.fn.countAll().as("count")), params))?.count ?? null) ?? 0;
}
...
function nextCronRunLogSeq(db, storeKey, jobId) {
	return (normalizeNumber(executeSqliteQueryTakeFirstSync(db, getCronRunLogKysely(db).selectFrom("cron_run_logs").select((eb) => eb.fn.max("seq").as("seq")).where("store_key", "=", storeKey).where("job_id", "=", jobId))?.seq ?? null) ?? 0) + 1;
}
...
async function appendCronRunLog(params) {
	...
	const next = (writesByTarget.get(writeKey) ?? Promise.resolve()).catch(() => void 0).then(async () => {
		runOpenClawStateWriteTransaction(({ db }) => {
			...
			if (params.opts?.keepLines !== false) pruneCronRunLogRows(db, storeKey, params.entry.jobId, params.opts?.keepLines ?? 2e3);
		});
	});
