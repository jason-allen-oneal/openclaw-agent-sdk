...
function loadIntegrityManifest(packagePath) {
    const integrityPath = resolve(packagePath, "openclaw.integrity.json");
    if (!existsSync(integrityPath))
        return null;
    return JSON.parse(readFileSync(integrityPath, "utf8"));
}
 ...
 */
function removeCopiedFiles(manifest, packagePath, workspacePath, integrity) {
    const removed = [];
    const skipped = [];
 ...
 */
function unregisterPackage(packageName, workspacePath) {
 ...
 */
function removeGeneratedFile(filename, workspacePath) {
...
export const disableCommand = new Command("disable")
    ...
    const generatedCount = [configRemoved, bindingsRemoved, schedulesRemoved].filter(Boolean).length;
    if (generatedCount > 0) {
