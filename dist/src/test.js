...
export class MockTools {
    ...
    dispatch(name, input) {
        ...
        if (!allowed) {
            const record = { name, input, blocked: true, error: "tool denied: " + name };
            this.calls.push(record);
            throw new Error("tool denied: " + name);
        }
        const record = { name, input, blocked: false, result: toolConfig?.result };
        this.calls.push(record);
        if (toolConfig?.error)
...
function resultFrom(condition, ok, bad) {
...
function verifyPackHashes(packagePath, manifest, integrity) {
    const errors = [];
    if (!integrity)
        return ["openclaw.integrity.json not found"];
    if (integrity.package.name !== manifest.name)
        errors.push("integrity package name mismatch");
    if (integrity.package.version !== manifest.version)
        errors.push("integrity package version mismatch");
    for (const entry of manifest.files.copy) {
        ...
        if (actual !== expected)
            errors.push(`hash mismatch: ${entry.dest}`);
    }
    for (const [dest, expected] of Object.entries(integrity.files)) {
        ...
        const actual = hashFile(resolve(packagePath, entry.src));
        if (actual !== expected)
            errors.push(`hash mismatch: ${dest}`);
    }
    for (const skill of manifest.skills ?? []) {
        ...
        if (actual !== expected)
            errors.push(`skill hash mismatch: ${skillPath}`);
    }
    return errors;
}
function verifyMismatchFailsClosed(packagePath, manifest, integrity) {
    if (!integrity)
        return fail("cannot tamper integrity because openclaw.integrity.json is missing");
    const firstTracked = manifest.files.copy.find((entry) => integrity.files[entry.dest]);
    if (!firstTracked)
        return fail("cannot exercise mismatch: package has no tracked files");
    const tampered = {
        ...integrity,
        files: { ...integrity.files, [firstTracked.dest]: "sha256:tampered" },
    };
    const errors = verifyPackHashes(packagePath, manifest, tampered);
    return resultFrom(errors.some((error) => error.includes(firstTracked.dest) || error.includes("hash mismatch")), "tampered integrity hash was rejected", "tampered integrity hash was accepted");
}
function copyTrackedFilesToWorkspace(packagePath, manifest, workspacePath) {
    for (const entry of manifest.files.copy) {
...
function verifyQuarantine(packagePath, manifest, integrity) {
    if (!integrity)
        return fail("cannot exercise quarantine because openclaw.integrity.json is missing");
    const firstTracked = manifest.files.copy[0];
    if (!firstTracked)
        return fail("cannot exercise quarantine: package has no tracked files");
    ...
    try {
        ...
        return resultFrom(!mutation.clean && mutation.mutated.length > 0 && record.mutations.length > 0, "tracked file drift was detected and quarantined", "tracked file drift did not trigger quarantine");
    }
    finally {
        rmSync(workspacePath, { recursive: true, force: true });
    }
}
function getNetworkPolicy(manifest) {
    return manifest.tools?.sandbox?.network ?? { egress: "restricted", denyPrivateRanges: true };
}
function verifyRequiredSecrets(manifest) {
    ...
    return resultFrom(missing.length === 0, `required secret mappings present: ${requiredSecrets.length}`, `required secret mappings missing: ${missing.map((secret) => secret.name).join(", ")}`);
}
function verifySecretFailsClosed() {
    const result = resolveSecret({ source: "env", key: "OPENCLAW_AGENT_SDK_PROOF_MISSING_SECRET" });
    return resultFrom(result.value === undefined && result.error !== undefined, "missing secret resolution failed closed", "missing secret resolution did not fail closed");
}
function verifySecretScope(manifest) {
    ...
    return resultFrom(isToolAllowed(allowedTool, allow, deny) && !isToolAllowed(deniedTool, allow, deny), `tool-scoped secret policy allowed ${allowedTool} and denied ${deniedTool}`, "tool-scoped secret policy did not enforce allow/deny lists");
}
function verifyNetworkPolicy(manifest) {
    ...
    return resultFrom(allowed.allowed && !denied.allowed, `network policy allowed ${allowedDomain} and denied unlisted egress`, `network policy failed: allowed=${allowed.allowed} denied=${denied.allowed}`);
}
function verifyPrivateNetworkBlocked(manifest) {
...
function verifyDnsRebindingBlocked(manifest) {
    const policy = { ...getNetworkPolicy(manifest), denyPrivateRanges: true };
    const domain = policy.allowedDomains?.[0]?.replace(/^\*\./, "api.") ?? "api.example.com";
...
function verifyTools(manifest) {
    ...
    return resultFrom(!isToolAllowed(denied, allow, deny), `denied tool was blocked: ${denied}`, `denied tool was allowed: ${denied}`);
}
function verifyExternalContentToExecBlocked() {
    ...
    return resultFrom(tools.getCallsFor("exec").some((call) => call.blocked), "external-content induced exec call was blocked", "external-content induced exec call was not blocked");
}
export async function runBehaviorProofs(packagePath) {
    ...
    try {
        ...
        manifest = null;
        validation.errors.push({ path: "agent-package.json", message: e.message });
    }
    const proofResults = new Map();
    proofResults.set("agent.manifest.valid", resultFrom(manifest !== null && validation.errors.filter((e) => e.path === "agent-package.json").length === 0, "manifest schema and package paths validated", formatErrors(validation.errors.filter((e) => e.path === "agent-package.json"))));
    if (!manifest) {
        for (const id of REQUIRED_V1_PROOF_IDS) {
            if (!proofResults.has(id))
                proofResults.set(id, fail("agent-package.json could not be loaded"));
        }
    }
    else {
        const hashErrors = verifyPackHashes(resolved, manifest, integrity);
        proofResults.set("agent.integrity.valid", resultFrom(hashErrors.length === 0, "integrity hashes match package contents", hashErrors.join("; ")));
        proofResults.set("agent.integrity.mismatchFailsClosed", verifyMismatchFailsClosed(resolved, manifest, integrity));
        proofResults.set("agent.installedState.valid", resultFrom(validation.valid, "validation completed with no errors", formatErrors(validation.errors)));
        ...
        proofResults.set("agent.mutableInstructionFile.deniedByPolicy", resultFrom(validation.errors.every((e) => !e.message.includes("mutable path")), "mutable instruction policy validated", formatErrors(validation.errors.filter((e) => e.message.includes("mutable path")))));
        proofResults.set("agent.requiredTool.missingFailsClosed", resultFrom(manifest.tools?.allow ? !isToolAllowed("__missing_required_tool__", manifest.tools.allow, manifest.tools.deny) : true, "missing required tool would fail closed under allow/deny policy", "missing required tool was allowed by policy"));
        proofResults.set("agent.requiredPlugin.missingFailsClosed", resultFrom((manifest.skills ?? []).every((skill) => skill.required === false || existsSync(resolve(resolved, `${skill.path}/SKILL.md`))), "required skill/plugin files are present and checked", "required skill/plugin file missing"));
        const requiredSecrets = verifyRequiredSecrets(manifest);
        const missingSecret = verifySecretFailsClosed();
        proofResults.set("agent.requiredSecret.missingFailsClosed", resultFrom(requiredSecrets.passed && missingSecret.passed, `${requiredSecrets.evidence}; ${missingSecret.evidence}`, `${requiredSecrets.evidence}; ${missingSecret.evidence}`));
        ...
        proofResults.set("agent.outbound.requiresApproval", verifyNetworkPolicy(manifest));
        proofResults.set("agent.workspaceEscape.blocked", resultFrom(validation.errors.every((e) => !e.message.includes("escapes")), "workspace path validation rejected escapes", formatErrors(validation.errors.filter((e) => e.message.includes("escapes")))));
        proofResults.set("agent.schedule.disabledByDefault", resultFrom((manifest.schedules ?? []).every((schedule) => schedule.sessionTarget !== "current"), "schedules default to isolated sessions", "schedule targets current session by default"));
        ...
        proofResults.set("agent.sandbox.required", resultFrom(manifest.tools?.sandbox?.mode !== "none", "sandbox policy is present or inherited", "sandbox disabled"));
        const diff = compileManifest(manifest, { strict: false });
        proofResults.set("agent.configCompiler.dryRunValidates", resultFrom(Object.keys(diff.changes).length > 0 && diff.unsupported.length === 0, "config compiler dry-run produced a valid package diff", `config compiler reported unsupported fields: ${diff.unsupported.join(", ")}`));
        ...
        proofResults.set("agent.upgrade.permissionExpansionRequiresApproval", resultFrom(upgrade.preserved.length > 0 || upgradeValidation.safe, "upgrade diff preserves policy changes unless reset is requested", upgradeValidation.warnings.join("; ")));
    }
...
export function formatBehaviorProofSummary(summary) {
    const lines = [
