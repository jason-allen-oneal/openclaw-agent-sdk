...
function parseSource(mapping) {
    if (mapping.source === "env")
        return { type: "env", key: mapping.key };
    if (mapping.source === "gateway")
        return { type: "gateway", ref: mapping.ref };
 ...
 * Never throws.
 */
export function resolveSecret(mapping, workspacePath = "") {
    try {
        ...
        return { value: undefined, error: `secret resolution error: ${e.message}` };
    }
}
/**
 * Check whether a tool is in the allowed list for a given resource.
 */
export function isToolAllowed(toolName, allowList, globalDeny) {
    // If global deny list exists and includes this tool, block.
    if (globalDeny?.includes(toolName))
        return false;
    // If allow list exists and doesn't include this tool, block.
    if (allowList !== undefined && !allowList.includes(toolName))
        return false;
    return true;
}
